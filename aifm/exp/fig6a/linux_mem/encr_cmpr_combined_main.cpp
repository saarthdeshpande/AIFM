...
private:
  ...
  uint32_t encr_compr_lats[helpers::kNumCPUs][kLatsWinSize];
    ...
    if (!flag.test_and_set()) {
      ...
      if (us - prev_us > kMaxPrintIntervalUs) {
        ...
        if (print_times++ >= kPrintTimes) {
          ...
          std::vector<uint32_t> all_encr_compr_lats;
          for (uint32_t i = 0; i < helpers::kNumCPUs; i++) {
            auto num_lats = std::min((uint64_t)kLatsWinSize, lats_idx[i].c);
            all_lats.insert(all_lats.end(), &lats[i][0], &lats[i][num_lats]);
            all_encr_compr_lats.insert(all_encr_compr_lats.end(), &encr_compr_lats[i][0], &encr_compr_lats[i][num_lats]);
          }
          std::sort(all_lats.begin(), all_lats.end());
          std::sort(all_encr_compr_lats.begin(), all_encr_compr_lats.end());
          ...
          std::cout << "90 tail lat (cycles) = "
            ...
            std::cout << "50 tail encr+cmpr lat (cycles) = "
                      << all_encr_compr_lats[all_encr_compr_lats.size() * 50.0 / 100] << std::endl;
          std::cout << "90 tail encr+cmpr lat (cycles) = "
                    << all_encr_compr_lats[all_encr_compr_lats.size() * 90.0 / 100] << std::endl;
          exit(0);
        }
        prev_us = us;
      ...
      threads.emplace_back(rt::Thread([&, tid]() {
        ...
        while (1) {
          ...
          }
          {
            ...
            auto end_cmpr = rdtsc();
            encr_compr_lats[core_num][(lats_idx[core_num].c++) % kLatsWinSize] = end_cmpr - start_cmpr;
            preempt_enable();
          }
          auto end = rdtsc();
          preempt_disable();
