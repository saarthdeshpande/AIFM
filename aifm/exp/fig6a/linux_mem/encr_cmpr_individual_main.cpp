...
namespace far_memory {
    ...
    private:
        ...
        // Runtime.
        constexpr static uint32_t kNumMutatorThreads = 11;
        ...
        uint32_t encr_lats[helpers::kNumCPUs][kLatsWinSize];
        uint32_t compr_lats[helpers::kNumCPUs][kLatsWinSize];
        ...
        void consume_array_entry(const ArrayEntry &entry) {
            ...
            auto end_encr = rdtsc();
            encr_lats[core_num][(lats_idx[core_num].c++) % kLatsWinSize] = end_encr - start_encr;
            auto start_cmpr = rdtsc();
            ...
            if (!flag.test_and_set()) {
                ...
                if (us - prev_us > kMaxPrintIntervalUs) {
                    ...
                    if (print_times++ >= kPrintTimes) {
                        ...
                        std::vector<uint32_t> all_encr_lats;
                        std::vector<uint32_t> all_cmpr_lats;
                        for (uint32_t i = 0; i < helpers::kNumCPUs; i++) {
                            auto num_lats = std::min((uint64_t)kLatsWinSize, lats_idx[i].c);
                            all_lats.insert(all_lats.end(), &lats[i][0], &lats[i][num_lats]);
                            all_encr_lats.insert(all_encr_lats.end(), &encr_lats[i][0], &encr_lats[i][num_lats]);
                            all_cmpr_lats.insert(all_cmpr_lats.end(), &compr_lats[i][0], &compr_lats[i][num_lats]);
                        }
                        std::sort(all_lats.begin(), all_lats.end());
                        std::sort(all_encr_lats.begin(), all_encr_lats.end());
                        std::sort(all_cmpr_lats.begin(), all_cmpr_lats.end());
                        std::cout << "50 tail lat (cycles) = "
                                  << all_lats[all_lats.size() * 50.0 / 100] << std::endl;
                        std::cout << "90 tail lat (cycles) = "
                                  << all_lats[all_lats.size() * 90.0 / 100] << std::endl;
                        std::cout << "50 tail encr lat (cycles) = "
                                  << all_encr_lats[all_encr_lats.size() * 50.0 / 100] << std::endl;
                        std::cout << "90 tail encr lat (cycles) = "
                                  << all_encr_lats[all_encr_lats.size() * 90.0 / 100] << std::endl;
                        std::cout << "50 tail cmpr lat (cycles) = "
                                  << all_cmpr_lats[all_cmpr_lats.size() * 50.0 / 100] << std::endl;
                        std::cout << "90 tail cmpr lat (cycles) = "
                                  << all_cmpr_lats[all_cmpr_lats.size() * 90.0 / 100] << std::endl;
                        exit(0);
                    }
                    prev_us = us;
